...
// 在两个子页（src/config、src/usage）内可直接引用，无需逐文件 import —— 这是 vue-tsc /
// vite build 最简单可靠的写法。若 addon-dev/sdk 的公共类型演进，保持此处字段一致即可。
...
/** 配置数据访问（FR-7 / 告警配置）：参数 + 告警配置读写（config 页消费） */
...
/** 运行时 SDK 形态（window.addonSDK）：取 config / usage 两页字段的并集 */
interface AddonSDK {
