...
//   user-editable        → ElInput 文本输入（SECRET/密钥类自动转密码框）
...
// 密钥类参数（SECRET / PASSWORD / 结尾 _KEY）渲染为密码框，避免明文示人
const isSensitive = () =>
  !isHostDerived() &&
  !isEnum() &&
  /(SECRET|PASSWORD|_KEY)$/.test(props.def.name)
</script>
...
<template>
  <div class="param-row" :data-category="def.category">
    <div class="param-row__label">
      ...
      <el-tag v-else-if="isSensitive()" size="small" type="warning" effect="plain">密钥</el-tag>
    </div>

    <!-- user-editable：文本输入（密钥类用密码框） -->
    <el-input
      v-if="!isHostDerived() && !isEnum()"
      data-kind="text"
      :model-value="value"
      :type="isSensitive() ? 'password' : 'text'"
      :show-password="isSensitive()"
